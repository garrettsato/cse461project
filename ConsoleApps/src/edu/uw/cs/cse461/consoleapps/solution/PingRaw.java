...
public class PingRaw extends NetLoadableConsoleApp implements PingRawInterface {
	...
	public ElapsedTimeInterval udpPing(byte[] header, String hostIP, int udpPort, int socketTimeout, int nTrials) {
		for (int i = 0; i < nTrials; i++) {
			try {
				...
				doUdpPing(header, hostIP, udpPort, socketTimeout, nTrials);
				ElapsedTime.stop("PingRaw_UDPTotalDelay");
			} catch (SocketTimeoutException e) {
				ElapsedTime.abort("PingRaw_UDPTotalDelay");
				System.out.println("updPing timed out : " + e.getMessage());
			} catch (Exception e) {
				ElapsedTime.abort("PingRaw_UDPTotalDelay");
				System.out.println("udpPing Exception: " + e.getMessage());
			}
		}
	...
	public void doUdpPing(byte[] header, String hostIP, int udpPort, int socketTimeout, int nTrials) throws Exception {
		...
		if ( !rcvdHeader.equalsIgnoreCase(EchoServiceBase.RESPONSE_OKAY_STR) ) 
			throw new Exception("Bad returned header: got '" + rcvdHeader + "' but expected '" + EchoServiceBase.RESPONSE_OKAY_STR);
		/*
	...
	public ElapsedTimeInterval tcpPing(byte[] header, String hostIP, int tcpPort, int socketTimeout, int nTrials) {
		for (int i = 0; i < nTrials; i++) {
			try {
				...
				doTcpPing(header, hostIP, tcpPort, socketTimeout, nTrials);
	...
	public void doTcpPing(byte[] header, String hostIP, int tcpPort, int socketTimeout, int nTrials) throws Exception {
